...
class MainDAO {
  ...
  updateFromStripe = async (id, status) => {
    ...
    await this.DonationData.findOneAndUpdate(
      { id: id },
  ...
  updateStudent = async (student) => {
    try {
      ...
      await this.StudentData.findByIdAndUpdate(id, {
        startDate: student.startDate,
        status: student.status,
        paid: student.paid,
      });
  ...
  addDonation = async (email, fullName, amount) => {
    try {
      ...
      await this.UserData.findOneAndUpdate(
        { email: email },
        { donations: donations }
      );
      return id;
  ...
  addPurchase = async (cart) => {
    try {
      ...
      console.log("addDonation.user:", cart.email, user);
      const userId = user ? user.userId : "";
      let amount = 0;
  ...
  getStudents = async (id): Promise<Array<IStudent>> => {
    const query = id === 0 ? {} : { _id: id };
    console.log("getStudents v2:", query);
    const data: Array<IStudent> = await this.StudentData.find(query);
    //const donations = data ? data.donations : [];
    return data;
  };
  getVideos = async (id) => {
